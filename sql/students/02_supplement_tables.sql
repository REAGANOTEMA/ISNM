-- ============================================================
-- ISNM STUDENTS DB - SUPPLEMENT TABLES
-- Run in: igangaschoolofl_students_db
-- ============================================================
USE `igangaschoolofl_students_db`;

-- Activity log for academic registrar actions
...
-- Trash bin for soft-deleted students
CREATE TABLE IF NOT EXISTS `students_trash` (
  ...
  `snapshot_data` longtext COLLATE utf8mb4_unicode_ci COMMENT 'JSON snapshot of full row',
...
-- Hostel allocations for students
...
-- Student requests (leave of absence, deferral, etc.)
...
-- Timetable (shared across programs)
...
CREATE TABLE IF NOT EXISTS `library_fines` (
  ...
  `days_overdue` int GENERATED ALWAYS AS (DATEDIFF(IFNULL(`return_date`, CURDATE()), `due_date`)) STORED,
...
SELECT 'Students DB supplement tables created successfully' AS status;
