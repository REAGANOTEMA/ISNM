...
$payments = [];
if ($studentsDb) {
    $r = $studentsDb->query("SELECT * FROM payments ORDER BY payment_date DESC LIMIT 20");
    if ($r && !($r === false)) {
...
<title>Payment Recording - ISNM</title>
...
<div class="main-content" style="margin-left:270px;padding:20px;background:#f0f2f5;min-height:100vh;">
    <div class="container-fluid">
        ...
        <div class="card-section text-center py-5 mb-4">
            <div class="coming-soon-icon mb-3"><i class="fas fa-credit-card"></i></div>
            <h5>Payment Recording</h5>
            <p class="text-muted">This module is under development. Payment capture, receipt generation, mobile money integration, and payment reconciliation will be available soon.</p>
        ...
        <div class="card-section">
            ...
            <div class="table-responsive">
                <table class="table table-hover table-bordered align-middle">
                    <thead class="table-light">
                        <tr>
                            <?php $cols = array_keys($payments[0]); foreach ($cols as $col): ?>
            ...
            <p class="text-muted small mb-0">Showing <?= count($payments) ?> recent payment(s).</p>
        </div>
        <?php else: ?>
        <div class="card-section">
            <div class="text-center py-4 text-muted">
                <i class="fas fa-database fa-2x mb-2"></i>
                <p class="mb-0">No payments found in the database yet.</p>
            </div>
        </div>
        <?php endif; ?>
    </div>
