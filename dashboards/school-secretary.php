...
if (!isset($_SESSION['user_id']) || $_SESSION['user_role'] !== 'School Secretary') {
...
$pending_letters = $conn->query("SELECT COUNT(*) as count FROM correspondence WHERE status = 'pending'")->fetch_assoc()['count'];
...
$meetings_scheduled = $conn->query("SELECT COUNT(*) as count FROM meetings WHERE status = 'scheduled'")->fetch_assoc()['count'];

// Get recent secretary activities
$recent_activities = $conn->query("SELECT * FROM secretary_logs ORDER BY created_at DESC LIMIT 10")->fetch_all(MYSQLI_ASSOC);
?>
