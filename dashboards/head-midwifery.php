            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        $staff_list = [];
                        if ($conn) {
                            $sr = $conn->query("SELECT full_name, position, email FROM staff WHERE department LIKE '%Midwifery%' ORDER BY full_name");
                            if ($sr) $staff_list = $sr->fetch_all(MYSQLI_ASSOC);
                        }
                        if (empty($staff_list)): ?>
                        <tr><td colspan="3" class="text-center text-muted">No midwifery staff found</td></tr>
                        <?php else: ?>
