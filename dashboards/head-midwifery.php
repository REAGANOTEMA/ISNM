...
if (!isset($_SESSION['user_id']) || $_SESSION['user_role'] !== 'Head of Midwifery') {
...
$midwifery_students = $conn->query("SELECT COUNT(*) as count FROM students WHERE program LIKE '%Midwifery%'")->fetch_assoc()['count'];
$midwifery_lecturers = $conn->query("SELECT COUNT(*) as count FROM users WHERE role LIKE '%Lecturer%' AND department = 'Midwifery'")->fetch_assoc()['count'];
$midwifery_courses = $conn->query("SELECT COUNT(*) as count FROM courses WHERE department = 'Midwifery' AND status = 'active'")->fetch_assoc()['count'];
$clinical_sites = $conn->query("SELECT COUNT(*) as count FROM clinical_sites WHERE department = 'Midwifery'")->fetch_assoc()['count'];

// Get recent midwifery activities
$recent_activities = $conn->query("SELECT * FROM midwifery_logs ORDER BY created_at DESC LIMIT 10")->fetch_all(MYSQLI_ASSOC);
?>
