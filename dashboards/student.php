...
if (tableExists($studentsDb, 'announcements')) {
    $ann_result = $studentsDb->query("SELECT a.*, COALESCE(s.full_name, CONCAT(s.first_name, ' ', s.surname)) AS posted_by_name FROM announcements a LEFT JOIN igangaschoolofl_staffs_db.staff s ON a.posted_by = s.id WHERE a.status = 'published' AND (a.target_audience = 'all' OR a.target_audience = 'students') AND (a.expiry_date IS NULL OR a.expiry_date >= CURDATE()) ORDER BY a.priority DESC, a.posted_date DESC LIMIT 5");
