...
// ─── Ensure tables exist ───
if ($staff_conn) {
    @$staff_conn->query("CREATE TABLE IF NOT EXISTS sickness_directory (id INT AUTO_INCREMENT PRIMARY KEY, sickness_code VARCHAR(50) DEFAULT '', sickness_name VARCHAR(200) DEFAULT '', category VARCHAR(100) DEFAULT '', common_symptoms TEXT, description TEXT, is_contagious TINYINT(1) DEFAULT 0, typical_treatment TEXT, status VARCHAR(50) DEFAULT 'Active', created_by INT DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    @$staff_conn->query("CREATE TABLE IF NOT EXISTS daily_sick_records (id INT AUTO_INCREMENT PRIMARY KEY, record_number VARCHAR(50) DEFAULT '', student_id INT DEFAULT 0, student_name VARCHAR(200) DEFAULT '', student_number VARCHAR(50) DEFAULT '', program VARCHAR(200) DEFAULT '', year_of_study INT DEFAULT 0, sickness_id INT DEFAULT NULL, sickness_name VARCHAR(200) DEFAULT '', temperature VARCHAR(20) DEFAULT '', blood_pressure VARCHAR(20) DEFAULT '', symptoms TEXT, diagnosis TEXT, treatment_given TEXT, medicines_prescribed TEXT, severity VARCHAR(50) DEFAULT '', status VARCHAR(50) DEFAULT '', referred_to VARCHAR(200) DEFAULT '', attended_by VARCHAR(200) DEFAULT '', visit_date DATE DEFAULT NULL, visit_time TIME DEFAULT NULL, follow_up_date DATE DEFAULT NULL, notes TEXT, is_deleted TINYINT(1) DEFAULT 0, deleted_at DATETIME DEFAULT NULL, created_by INT DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    @$staff_conn->query("CREATE TABLE IF NOT EXISTS student_sick_leave (id INT AUTO_INCREMENT PRIMARY KEY, leave_number VARCHAR(50) DEFAULT '', leave_type VARCHAR(50) DEFAULT 'Sick', student_id INT DEFAULT 0, student_name VARCHAR(200) DEFAULT '', student_number VARCHAR(50) DEFAULT '', program VARCHAR(200) DEFAULT '', year_of_study INT DEFAULT 0, sickness_id INT DEFAULT NULL, sickness_name VARCHAR(200) DEFAULT '', leave_from DATE DEFAULT NULL, leave_to DATE DEFAULT NULL, status VARCHAR(50) DEFAULT 'Pending', recommended_by VARCHAR(200) DEFAULT '', bed_rest_required TINYINT(1) DEFAULT 0, doctor_notes TEXT, is_deleted TINYINT(1) DEFAULT 0, deleted_at DATETIME DEFAULT NULL, created_by INT DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    @$staff_conn->query("CREATE TABLE IF NOT EXISTS sickbay_medicine_stock (id INT AUTO_INCREMENT PRIMARY KEY, medicine_name VARCHAR(200) DEFAULT '', medicine_code VARCHAR(50) DEFAULT '', generic_name VARCHAR(200) DEFAULT '', category VARCHAR(100) DEFAULT '', dosage_form VARCHAR(100) DEFAULT '', strength VARCHAR(100) DEFAULT '', manufacturer VARCHAR(200) DEFAULT '', supplier VARCHAR(200) DEFAULT '', quantity INT DEFAULT 0, unit VARCHAR(50) DEFAULT 'Tablets', reorder_level INT DEFAULT 10, unit_cost DECIMAL(14,2) DEFAULT 0, selling_price DECIMAL(14,2) DEFAULT 0, currency VARCHAR(10) DEFAULT 'UGX', batch_number VARCHAR(100) DEFAULT '', expiry_date DATE DEFAULT NULL, storage_location VARCHAR(200) DEFAULT '', requires_prescription TINYINT(1) DEFAULT 0, instructions TEXT, side_effects TEXT, status VARCHAR(50) DEFAULT 'In Stock', last_restocked DATE DEFAULT NULL, created_by INT DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
...
if ($action_get === 'get_visit' && $staff_conn) {
    ...
    if ($vid > 0) {
        ...
        if ($stmt) { $stmt->bind_param('i', $vid); if (!$stmt->execute()) { error_log('$stmt execute failed: ' . ($stmt->error ?? 'unknown')); }; $r = $stmt->get_result(); $stmt->close(); } else $r = null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $staff_conn) {
    ...
    if ($action === 'stock_transaction') {
        ...
        if ($mid > 0 && $qty > 0) {
            ...
            if ($stmt) { $stmt->bind_param('i', $mid); if (!$stmt->execute()) { error_log('$stmt execute failed: ' . ($stmt->error ?? 'unknown')); }; $qrMed = $stmt->get_result(); $stmt->close(); } else $qrMed = null;
            ...
            if ($med) {
                ...
                if ($stmt) { $stmt->bind_param('i', $mid); if (!$stmt->execute()) { error_log('$stmt execute failed: ' . ($stmt->error ?? 'unknown')); }; $qrRl = $stmt->get_result(); $stmt->close(); } else $qrRl = null;
    ...
    if ($action === 'save_health_record') {
        ...
        if ($stmt) { $stmt->bind_param('i', $sid); if (!$stmt->execute()) { error_log('$stmt execute failed: ' . ($stmt->error ?? 'unknown')); }; $existing = $stmt->get_result(); $stmt->close(); }
        else $existing = null;
    ...
    if ($action === 'dispense_medicine') {
        ...
        if ($mid > 0 && $qty > 0) {
            ...
            if ($stmt) { $stmt->bind_param('i', $mid); if (!$stmt->execute()) { error_log('$stmt execute failed: ' . ($stmt->error ?? 'unknown')); }; $qr = $stmt->get_result(); $stmt->close(); } else $qr = null;
...
<div class="top-bar"><div><strong><i class="fas fa-hospital-user me-2 text-danger"></i>Sickbay Management System</strong><div class="text-muted small">Iganga School of Nursing &amp; Midwifery</div></div><div class="d-flex align-items-center gap-3"><span class="text-muted small d-none d-md-block"><?=date('D, d M Y')?></span><a href="inventory-reports.php" class="btn btn-sm btn-outline-info no-print" title="Reports"><i class="fas fa-chart-bar me-1"></i></a><button class="btn btn-sm btn-outline-success no-print" onclick="window.print()"><i class="fas fa-print me-1"></i></button><a href="#" class="btn btn-sm btn-outline-danger no-print" onclick="event.preventDefault();var f=document.createElement('form');f.method='POST';f.action='../auth-handler.php?action=logout';document.body.appendChild(f);f.submit();"><i class="fas fa-sign-out-alt me-1"></i>Logout</a></div></div>
...
<?php $audit_logs = sb_fetch($staff_conn, "SELECT a.*, u.full_name AS actor_name FROM (SELECT id, 'daily_sick_records' AS log_source, CONCAT(student_name, ' - ', diagnosis) AS description, 'Record' AS log_type, created_at, created_by FROM daily_sick_records WHERE created_by IS NOT NULL UNION ALL SELECT id, 'student_sick_leave' AS log_source, CONCAT(student_name, ' - ', leave_type, ' leave') AS description, 'Leave' AS log_type, created_at, created_by FROM student_sick_leave WHERE created_by IS NOT NULL UNION ALL SELECT id, 'sickbay_medicine_stock' AS log_source, medicine_name AS description, 'Stock' AS log_type, created_at, created_by FROM sickbay_medicine_stock WHERE created_by IS NOT NULL UNION ALL SELECT mst.id, 'sickbay_medicine_transactions' AS log_source, CONCAT('Transaction: ', mst.transaction_type) AS description, 'Transaction' AS log_type, mst.created_at, mst.performed_by FROM sickbay_medicine_transactions mst ) a LEFT JOIN staff_profiles u ON a.created_by = u.id ORDER BY a.created_at DESC LIMIT 200"); ?>
