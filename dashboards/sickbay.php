...
<div class="top-bar"><div><strong><i class="fas fa-hospital-user me-2 text-danger"></i>Sickbay Management System</strong><div class="text-muted small">Iganga School of Nursing &amp; Midwifery</div></div><div class="d-flex align-items-center gap-3"><span class="text-muted small d-none d-md-block"><?=date('D, d M Y')?></span><a href="inventory-reports.php" class="btn btn-sm btn-outline-info no-print" title="Reports"><i class="fas fa-chart-bar me-1"></i></a><button class="btn btn-sm btn-outline-success no-print" onclick="window.print()"><i class="fas fa-print me-1"></i></button><a href="../logout.php" class="btn btn-sm btn-outline-danger no-print"><i class="fas fa-sign-out-alt me-1"></i>Logout</a></div></div>
...
<?php foreach($recent_records as $r):$sc=match($r['severity']??''){'Mild'=>'bg-success','Moderate'=>'bg-warning text-dark','Severe'=>'bg-orange','Critical'=>'bg-danger',default=>'bg-secondary'};$st=match($r['status']??''){'Treated','Discharged'=>'bg-success','Referred'=>'bg-info text-dark','Admitted'=>'bg-warning text-dark','Critical'=>'bg-danger','Follow-up'=>'bg-primary',default=>'bg-secondary'};?>
...
<tr><td><small><?=$i++?></small></td><td><strong><small><?=htmlspecialchars($r['student_name']??$r['student_full_name']??'Unknown')?></small></strong><small class="d-block text-muted"><?=htmlspecialchars($r['student_number']??'')?></small></td><td><small><?=htmlspecialchars($r['sickness_name']??substr($r['diagnosis']??'N/A',0,40))?></small></td><td><span class="badge <?=match($r['severity']){'Mild'=>'bg-success','Moderate'=>'bg-warning text-dark','Severe'=>'bg-orange','Critical'=>'bg-danger',default=>'bg-secondary'}?>"><?=htmlspecialchars($r['severity']??'N/A')?></span></td><td><span class="badge <?=match($r['status']){'Treated','Discharged'=>'bg-success','Referred'=>'bg-info text-dark','Admitted'=>'bg-warning text-dark','Critical'=>'bg-danger','Follow-up'=>'bg-primary',default=>'bg-secondary'}?>"><?=htmlspecialchars($r['status']??'N/A')?></span></td><td><small><?=date('d M Y',strtotime($r['visit_date']))?></small></td><td><form method="POST" class="d-inline" onsubmit="return confirm('Delete?')"><input type="hidden" name="action" value="delete_sick_record"><input type="hidden" name="record_id" value="<?=$r['id']?>"><button class="btn btn-sm btn-outline-danger"><i class="fas fa-trash-alt"></i></button></form></td></tr>
...
<?php else:foreach($medicines as $md):$msc=match($md['status']){'In Stock'=>'bg-success','Low Stock'=>'bg-warning text-dark','Out of Stock'=>'bg-danger','Expired'=>'bg-secondary',default=>'bg-secondary'};$mcs=$md['status']==='Out of Stock'?'stock-critical':($md['status']==='Low Stock'?'stock-warning':($md['status']==='Expired'?'stock-critical':''));?>
