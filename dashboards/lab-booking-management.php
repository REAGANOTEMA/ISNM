...
function lb_q($conn, $sql) {
    ...
    catch (Exception $e) { error_log('lab-booking getCount: ' . $e->getMessage()); return 0; }
}
...
<div class="top-bar"><div><strong><i class="fas fa-calendar-alt me-2 text-primary"></i>Lab Booking Management</strong><div class="text-muted small">Iganga School of Nursing &amp; Midwifery</div></div><div class="d-flex align-items-center gap-3"><span class="text-muted small d-none d-md-block"><?=date('D, d M Y')?></span><button class="btn btn-sm btn-outline-success no-print" onclick="window.print()"><i class="fas fa-print me-1"></i></button><a href="#" class="btn btn-sm btn-outline-danger no-print" onclick="event.preventDefault();var f=document.createElement('form');f.method='POST';f.action='../logout.php';document.body.appendChild(f);f.submit();"><i class="fas fa-sign-out-alt me-1"></i>Logout</a></div></div>
