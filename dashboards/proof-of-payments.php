...
$records = [];
$r = $conn->query("SELECT p.*, s.first_name, s.last_name, s.student_number, py.payment_receipt FROM proof_of_payments p LEFT JOIN students s ON p.student_id=s.id LEFT JOIN payments py ON p.payment_id=py.id ORDER BY p.created_at DESC");
