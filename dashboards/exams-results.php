...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'exam_students' && isset($_GET['exam'])) {
    ...
    $data = [];
    if ($conn && $examNumber) {
        $r = $conn->query("SELECT er.id, er.student_id, er.continuous_assessment_marks, er.final_exam_marks, er.marks_obtained, er.grade, CONCAT(s.first_name,' ',s.surname) full_name, s.index_number, s.student_number FROM examination_records er JOIN students s ON er.student_id=s.id WHERE er.exam_number='$examNumber' ORDER BY s.surname, s.first_name");
...
if ($conn) {
    ...
    $sr = $conn->query("SELECT id, CONCAT(first_name,' ',surname) full_name, index_number, student_number FROM students WHERE status='Active' ORDER BY surname LIMIT 500");
