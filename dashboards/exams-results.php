    ...
    <table class="table table-striped table-hover">
      ...
      <tbody><?php if (empty($exams)): ?><tr><td colspan="6" class="text-muted text-center py-3">No exam records found.</td></tr><?php else: foreach ($exams as $e): ?><tr>
        ...
        <td class="no-print">
          ...
          <form method="POST" style="display:inline" onsubmit="return confirm('Submit for review?')"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>"><input type="hidden" name="action" value="update_status"><input type="hidden" name="exam_number" value="<?= htmlspecialchars($e['exam_number']) ?>"><input type="hidden" name="new_status" value="Submitted"><button class="btn btn-sm btn-outline-warning py-0 px-1" title="Submit"><i class="fas fa-paper-plane"></i></button></form>
          <?php elseif ($e['grade_status'] === 'Submitted'): ?>
          <form method="POST" style="display:inline" onsubmit="return confirm('Approve results?')"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>"><input type="hidden" name="action" value="update_status"><input type="hidden" name="exam_number" value="<?= htmlspecialchars($e['exam_number']) ?>"><input type="hidden" name="new_status" value="Approved"><button class="btn btn-sm btn-outline-success py-0 px-1" title="Approve"><i class="fas fa-check"></i></button></form>
          <?php elseif ($e['grade_status'] === 'Approved'): ?>
          <form method="POST" style="display:inline" onsubmit="return confirm('Publish results?')"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>"><input type="hidden" name="action" value="update_status"><input type="hidden" name="exam_number" value="<?= htmlspecialchars($e['exam_number']) ?>"><input type="hidden" name="new_status" value="Published"><button class="btn btn-sm btn-outline-success py-0 px-1" title="Publish"><i class="fas fa-globe"></i></button></form>
          <?php endif; ?>
          <form method="POST" style="display:inline" onsubmit="return confirm('Delete this exam?')"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>"><input type="hidden" name="action" value="delete_exam"><input type="hidden" name="exam_number" value="<?= htmlspecialchars($e['exam_number']) ?>"><button class="btn btn-sm btn-outline-danger py-0 px-1" title="Delete"><i class="fas fa-trash-alt"></i></button></form>
        </td>
...
<!-- Create Exam Modal -->
<div class="modal fade" id="examModal" tabindex="-1"><div class="modal-dialog modal-lg modal-dialog-scrollable"><form method="POST" class="modal-content"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>"><input type="hidden" name="action" value="create_exam"><div class="modal-header bg-primary text-white"><h5 class="modal-title">Create Exam</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body">
  <div class="row g-3">
