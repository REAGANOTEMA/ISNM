...
$migrate = function($db) use ($staff_db, $students_db) {
    ...
    $db->query("CREATE TABLE IF NOT EXISTS {$students_db}.budget_records (id INT AUTO_INCREMENT PRIMARY KEY, budget_code VARCHAR(50) DEFAULT '', budget_name VARCHAR(200) DEFAULT '', budget_category VARCHAR(100) DEFAULT '', department VARCHAR(100) DEFAULT '', fiscal_year VARCHAR(20) DEFAULT '', allocated_amount DECIMAL(14,2) DEFAULT 0, spent_amount DECIMAL(14,2) DEFAULT 0, status VARCHAR(50) DEFAULT 'Draft', created_by INT DEFAULT 0, approved_by INT DEFAULT 0, notes TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    $db->query("CREATE TABLE IF NOT EXISTS {$staff_db}.store_requests (id INT AUTO_INCREMENT PRIMARY KEY, request_number VARCHAR(50) DEFAULT '', requested_by INT DEFAULT 0, requester_name VARCHAR(255) DEFAULT '', requester_role VARCHAR(50) DEFAULT '', department VARCHAR(100) DEFAULT '', urgency VARCHAR(50) DEFAULT 'Normal', status VARCHAR(50) DEFAULT 'pending', notes TEXT, items TEXT, rejection_reason TEXT, fulfilled_by INT DEFAULT NULL, fulfilled_at DATETIME DEFAULT NULL, approved_by INT DEFAULT NULL, approved_at DATETIME DEFAULT NULL, approval_request_id INT DEFAULT NULL, forwarded_to INT DEFAULT NULL, forwarded_to_role VARCHAR(100) DEFAULT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, INDEX idx_sr_status (status), INDEX idx_sr_by (requested_by)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    $db->query("CREATE TABLE IF NOT EXISTS {$staff_db}.store_categories (id INT AUTO_INCREMENT PRIMARY KEY, category_name VARCHAR(200) DEFAULT '', description TEXT, status VARCHAR(20) DEFAULT 'active', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    $db->query("CREATE TABLE IF NOT EXISTS {$staff_db}.store_inventory (id INT AUTO_INCREMENT PRIMARY KEY, item_code VARCHAR(50) DEFAULT '', item_name VARCHAR(200) DEFAULT '', category_id INT DEFAULT NULL, unit VARCHAR(50) DEFAULT '', quantity DECIMAL(14,2) DEFAULT 0, reorder_level DECIMAL(14,2) DEFAULT 0, unit_cost DECIMAL(14,2) DEFAULT 0, location VARCHAR(200) DEFAULT '', batch_number VARCHAR(100) DEFAULT NULL, expiry_date DATE DEFAULT NULL, supplier VARCHAR(200) DEFAULT '', status VARCHAR(20) DEFAULT 'active', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    $db->query("CREATE TABLE IF NOT EXISTS {$staff_db}.store_request_items (id INT AUTO_INCREMENT PRIMARY KEY, request_id INT DEFAULT 0, item_id INT DEFAULT 0, quantity_requested DECIMAL(14,2) DEFAULT 0, quantity_fulfilled DECIMAL(14,2) DEFAULT 0, notes TEXT, status VARCHAR(50) DEFAULT 'pending', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
