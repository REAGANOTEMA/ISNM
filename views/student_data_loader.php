...
class StudentDataLoader {
    ...
    private function mapDbRowToStudent(array $row, $source) {
        ...
        return [
            ...
            'email' => $row['email'] ?? '',
            'passport_photo' => $row['passport_photo'] ?? $row['profile_picture'] ?? $row['photo'] ?? '',
            'profile_picture' => $row['profile_picture'] ?? $row['passport_photo'] ?? $row['photo'] ?? '',
            'program' => $row['program'] ?? $row['course'] ?? '',
            'level' => $row['level'] ?? '',
            'set' => $row['set_name'] ?? $row['set'] ?? $row['class_set'] ?? '',
            'set_name' => $row['set_name'] ?? $row['set'] ?? $row['class_set'] ?? '',
            'intake_year' => $row['intake_year'] ?? $row['year'] ?? $row['current_year'] ?? '',
            'intake_period' => $row['intake_period'] ?? '',
            'year' => $row['year'] ?? $row['current_year'] ?? '',
        ];
    }
    ...
    private function mapRowToStudent($row, $sourceFile, array $headers = []) {
        ...
        $student = [
            ...
            'mobile_number' => $this->firstNonEmpty([$this->cell($row, $headers, ['mobile_number', 'mobile', 'phone', 'phone_number', 'phone_no'])]),
            ...
            'set' => $this->firstNonEmpty([$this->cell($row, $headers, ['set', 'class_set', 'intake_set', 'set_name']), $this->extractSetFromFilename($sourceFile)]),
            'set_name' => $this->firstNonEmpty([$this->cell($row, $headers, ['set_name', 'set', 'class_set', 'intake_set']), $this->extractSetFromFilename($sourceFile)]),
